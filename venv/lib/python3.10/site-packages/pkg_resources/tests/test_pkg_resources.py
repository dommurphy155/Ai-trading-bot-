...
@pytest.mark.xfail(sys.version_info[:2] == (3,
                                            12) and sys.version_info.releaselevel != 'final',
...
class TestWorkdirRequire:
    ...
    def test_require_non_normalised_name(
        ...
        site_packages = self.fake_site_packages(
            tmp_path, monkeypatch, self.FILES)
        ws = pkg_resources.WorkingSet([site_packages])

        for req in [requirement, requirement.replace(".", "-")]:
            [dist] = ws.require(req)
            assert dist.version == version
            assert os.path.samefile(os.path.commonpath(
                [dist.location, site_packages]), site_packages)
