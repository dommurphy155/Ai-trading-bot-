...
class EarningsTracker:
    ...
    async def _load_data(self) -> None:
        """Load existing trades and performance data asynchronously"""
        try:
    ...
    async def _save_data(self) -> None:
        """Save trades and performance data asynchronously"""
        try:
    ...
    async def record_trade(self, trade_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.info(f"Trade recorded: {trade_data.get('symbol')} {trade_data.get('side')} {trade_data.get('volume')}")
        except Exception as e:
            self.logger.error(f"Error recording trade: {e}")

    async def update_trade_pnl(self, trade_id: str, current_pnl: float, current_price: Optional[float] = None) -> None:
        """Update trade P&L"""
        try:
    ...
    async def close_trade(self, trade_id: str, close_price: float, close_time: Optional[str] = None, final_pnl: Optional[float] = None) -> None:
        ...
        try:
            for trade in self.trades:
                if trade.get("trade_id") == trade_id:
                    ...
                    # Calculate duration in hours if possible
                    try:
            ...
            await self._save_data()
            await self._update_ai_insights()  # placeholder for future AI self-learning
    ...
    async def get_current_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            daily_trades = [
                t for t in closed_trades if datetime.fromisoformat(t.get("close_time", "")).date() == today
            ]
            daily_pnl = sum(t.get("pnl", 0) for t in daily_trades)
            week_ago = today - timedelta(days=7)
            weekly_trades = [
                t for t in closed_trades if datetime.fromisoformat(t.get("close_time", "")).date() >= week_ago
            ]
            ...
            return {
                ...
                "open_positions": len(open_trades),
                "avg_trade": total_pnl / max(len(closed_trades), 1),
            }
        except Exception as e:
